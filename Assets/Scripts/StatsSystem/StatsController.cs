...
{
    public class StatsController : IDisposable, IStatValueGiven
    {
        private readonly List<Stat> _currentStats;
        private readonly List<StatModificator> _activeModifiers;

        public StatsController(List<Stat> currentStats)
        {
            _currentStats = currentStats;
            _activeModifiers = new List<StatModificator>();
        ...
        public void ProcessModificator(StatModificator modificator)
        {
            ...
            if (_activeModifiers.Contains(modificator))
            {
                _activeModifiers.Remove(modificator);
            }
            else
            {
        ...
        private void OnUpdate()
        {
            ...
            var expiredModificator = _activeModifiers.Where(modificator => modificator.StartTime + modificator.Duration >= Time.time);
